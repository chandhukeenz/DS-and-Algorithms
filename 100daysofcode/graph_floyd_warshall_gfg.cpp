...
43.
Your Task:
You don't need to read, return or print anything. Your task is to complete the function shortest_distance()
...
public:
	void shortest_distance(vector<vector<int>>&matrix){
	    ...
	    for(int k=0;k<v;k++){
	        for(int i=0;i<v;i++){
	            for(int j=0;j<v;j++){
	                if(matrix[i][k]+matrix[k][j]<matrix[i][j] && matrix[i][k]>=0 &&matrix[k][j]>=0){
